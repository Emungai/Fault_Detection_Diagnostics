...
float_t ctrl_update_freq = 100; // float_t is the longer version of float see https://stackoverflow.com/questions/5390011/whats-the-point-of-float-t-and-when-should-it-be-used
mjtNum last_update = 0.0;
mjtNum ctrl;
mjtNum ctrl_flywheel;
...
void mycontroller(const mjModel* m, mjData* d)
{
    ...
    std::cout << "flywheel position: " << d->qpos[1] << std::endl;
    std::cout << "joint velocity: " << d->qvel[0] << std::endl;
    std::cout << "flywheel velocity: " << d->qvel[1] << std::endl;
    std::cout << "Sensor output: " << d->sensordata[0] << std::endl;
    std::cout << "Flywhell sensor output: " << d->sensordata[1] << std::endl;

    // controller with true values, but it is cheating.
    ctrl = 3.5*(-d->qvel[0]-10.0*d->qpos[0]);
    //ctrl = (0*-d->qvel[0]-50*d->qpos[0]);
    
    ctrl_flywheel = 1*(-d->qvel[1]-1*d->qpos[1]);
...
*/
   // d->ctrl[0] = ctrl;
    //d->ctrl[1] = ctrl_flywheel;
 d->ctrl[0] = 0;
    d->ctrl[1] = 1;
    std::cout << "torque effort: " << ctrl << std::endl;
    std::cout << "flywheel torque effort: " << ctrl << std::endl;
}
...
int main(int argc, const char** argv)
{
    ...
    // activate software
    mj_activate("/home/exo/Documents/mujoco200_linux/bin/mjkey.txt");
    ...
    if( argc<2 )
        m = mj_loadXML("/home/exo/Documents/eva/projects/mujoco/tutorial/invertedPendulumFlyWheel.xml", 0, error, 1000); //if no xml file is given load this file
    ...
    // make data
    d = mj_makeData(m); // d contains all the data needed for computation while m contains all the information of the model 
