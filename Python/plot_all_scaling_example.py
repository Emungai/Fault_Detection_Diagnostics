...
if exampleData:
    ...
    feat_info['forceAxis']="y"
    data=getDigitFeatures(feat_info)
    time_data = data['time_data']
    FDD_bc = data['FDD_bc']
    y_full= data['label'][:,0]

    pca=PCA(n_components=2) #setting up the PCA function and keeping all the principal components
     #applying PCA to L
    distributions = [
    ('Unscaled data', pca.fit_transform(FDD_bc)),
    ('Data after standard scaling',
        pca.fit_transform(StandardScaler().fit_transform(FDD_bc))),
    ('Data after min-max scaling',
        pca.fit_transform(MinMaxScaler().fit_transform(FDD_bc))),
    ('Data after max-abs scaling',
        MaxAbsScaler().fit_transform(FDD_bc)),
    ('Data after robust scaling',
        pca.fit_transform(RobustScaler(quantile_range=(25, 75)).fit_transform(FDD_bc))),
    ('Data after power transformation (Yeo-Johnson)',
     pca.fit_transform(PowerTransformer(method='yeo-johnson').fit_transform(FDD_bc))),
    ('Data after quantile transformation (uniform pdf)',
        pca.fit_transform(QuantileTransformer(output_distribution='uniform')
        .fit_transform(FDD_bc))),
    ('Data after quantile transformation (gaussian pdf)',
        pca.fit_transform(QuantileTransformer(output_distribution='normal')
        .fit_transform(FDD_bc))),
    ('Data after sample-wise L2 normalizing',
        pca.fit_transform(Normalizer().fit_transform(FDD_bc))),
]
